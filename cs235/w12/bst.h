...
public:
	...
	bool findBool(const T& data) { return findBoolAux(root, data); }
	BSTIterator<T>& begin();
	BSTIterator<T>& end();
...
private:
	...
	bool findBoolAux(BinaryNode<T>* subroot, const T& data);
...
/*template <class T>
void redBlackInsert(BinaryNode<T>*& node)
{
...
template<class T>
bool BST<T>::findBoolAux(BinaryNode<T>* subroot, const T & data)
{
	if (subroot == NULL)
		return false;
	else if (data < subroot->data)
	{
		findBoolAux(subroot->pLeft, data);
	}
	else if (subroot->data < data)
	{
		findBoolAux(subroot->pRight, data);
	}
	else
	{
		return true;
